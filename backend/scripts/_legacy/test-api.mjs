...
async function main() {
  ...
  console.log('Vehicles (from API):', vehiclesRes.body.data?.map(v => v.number) || 'none');

  if (vehiclesRes.body.data?.length > 0) {
    const num = vehiclesRes.body.data[0].number;
    console.log(`\nTesting resolve-number with: ${num}`);
    ...
    console.log('Result:', JSON.stringify(resolveRes.body, null, 2));
  }
}
