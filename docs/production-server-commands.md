...
После рестарта web проверьте, что HTML и `/_next/static/chunks` относятся к одной сборке:

```powershell
$env:WEB_SMOKE_BASE_URL='https://<project-domain>'
npm --prefix web run smoke:static-chunks
```

Если smoke падает на `/_next/static/chunks/*.js` с `404`, `500` или `text/plain`, значит на сервере смешались старая HTML/route-сборка и новые static assets. Повторите чистую сборку `web/.next`, перезапустите web-процесс и очистите CDN/browser cache для `/_next/static/*`.
