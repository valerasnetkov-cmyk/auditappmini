...
- **`DEFAULT_JWT_SECRET` fallback** в server.js:37 — оставлен с guard в production; удалить после введения secret store. ✅ **Закрыто**: создан `backend/src/services/secretStore.js` (`getSecret` с `allowRandomFallback` + `minLength: 32`); `server.js:22` импортирует `getSecret`; `DEFAULT_JWT_SECRET` удалён; `assertProductionConfig` использует hardcoded `unsafeJwtSecrets` Set; dev без `JWT_SECRET` генерирует random fallback (96 hex chars) с warning; production требует strong secret. `backend/.env.example:5-9` обновлён (`JWT_SECRET=` пустой).
...
- `docs/epics/server-decomposition.md` — Epic 3.3: декомпозиция `server.js` (3 367 строк).
