...
- **Vehicle overview thumbnails**: vehicle cards and the vehicle list now use
  only real overview inspection photos (`overview`, with legacy `overall`
...
- **Mobile OCR recognition flow**: в мобильном осмотре на шаге одометра
  ...
  подтверждения; backend-распознавание номера техники больше не является
  пустой заглушкой и при `tesseract-cli` извлекает кандидаты госномера через
  OCR с нормализацией кириллических символов.
- **Tenant navigation activity badges**: боковое меню компании теперь показывает
  менеджерам и владельцам счетчики новых осмотров и дефектов; бейджи строятся
  из `company_notifications`, автоматически помечаются прочитанными при входе
  в соответствующий раздел и создаются при новых осмотрах, ручных дефектах и
  дефектах из проваленных пунктов чек-листа.
- **Mobile photo geolocation capture**: мобильное приложение теперь пытается
  получить foreground-геолокацию при съёмке каждого фото осмотра и дефекта, а
  не только на шаге ДТП; координаты сохраняются в draft и отправляются как
  `captured_lat` / `captured_lng` вместе с фото.
- **Mobile inspection flow controls**: в мобильном осмотре добавлены стрелка
  “Назад” для возврата на предыдущий шаг и кнопка “Сбросить осмотр” с
  подтверждением; сброс очищает локальный черновик/фото и удаляет незавершённый
  серверный осмотр, чтобы данные текущего осмотра не сохранялись.
...
- **Inspection photo modal moved to tenant inspection page**: ошибочный доступ
  ...
  миниатюры обязательных фото на странице осмотра открываются в модальном окне.
  При открытии фото tenant-only API лениво формирует водяную версию с данными
  компании, техники, осмотра, времени и координатами; полная версия в новой
  вкладке также ведёт на watermarked-файл.
- **Pilot request admin actions**: в модальном окне заявки на пилот добавлены
  быстрые действия “Отклонить” с обязательной причиной и “Добавить компанию”,
  которое переводит на вкладку компаний с предзаполненным названием из заявки.
...
- **Secure report links and photo evidence viewer**: PDF QR-ссылки теперь
  ...
  публичный PDF закрыт по умолчанию и включается только opt-in; общий helper
  водяных знаков для отчётов включает координаты фото при наличии.
...
- **Заявки на пилот**: пилотные CTA лендинга переведены с `mailto` на
  ...
  счётчиком новых обращений, URL-фильтрами, ответственными, статусами,
  внутренними комментариями и permissions для support/marketing. Одобренная
  заявка преобразуется транзакционным мастером в компанию на тарифе `pilot`,
  владельца и setup-ссылку. Дубли за 24 часа подавляются, отклонённые заявки
  автоматически обезличиваются через 12 месяцев, действия пишутся в
  `audit_logs` без контактных данных.
- **Меню столбцов техники**: выпадающий блок выбора видимых столбцов теперь
  отображается поверх закреплённого заголовка таблицы и больше им не
  перекрывается.
- **Расширение SaaS-админки**: существующий `/saas-admin` дополнен серверным
  реестром компаний с URL-фильтрами, транслитерируемыми уникальными slug,
  реквизитами компаний, предпросмотром тарифных лимитов, автозаполнением
  офлайн-платежей и отображением автора платежа. Добавлены сотрудники
  `resource_manager` без `company_id`, индивидуальные permissions и пресеты
  `support` / `finance` / `marketing` / `custom`, раздел «Команда сервиса»,
  шаблоны и ручные сервисные сообщения, а также singleton-профиль сервиса в
  `/profile`. Resource managers блокируются на tenant API, финансовые поля
  фильтруются по permissions, а действующие планы, лимиты, платежи,
  подписки и billing alerts сохранены без дублирования.
- **SaaS-admin reliability follow-up**: сохранение индивидуальных permissions
  больше не активирует отключённого сотрудника, начальный пресет корректно
  заполняет права, обновление компаний сохраняет текущие URL-фильтры, а выбор
  адресата сервисного сообщения включает владельца и назначенных получателей
  сервисных уведомлений. Транзитивный `shell-quote` в mobile lockfile обновлён
  с `1.8.3` до `1.8.4`, устранив критическое предупреждение dependency audit.
- **Единый lint-контур**: backend и mobile получили ESLint 9 flat configs и
  собственные `lint`-команды; корневой `npm run lint` последовательно
  проверяет backend, web и mobile, а `npm run verify` теперь включает эту
  проверку. В mobile исправлены нестабильные auth/theme effects, зависимости
  camera-permission effect и неиспользуемые импорты; mobile lint и typecheck
  проходят без предупреждений.
- **Lint cleanup**: активный backend очищен от неиспользуемых параметров,
  импортов и состояния, архивные `scripts/_legacy` исключены из рабочей
  проверки. Защищённые thumbnail-фото осмотров переведены на `next/image`
  с `unoptimized`, чтобы сохранить cookie-auth запросы без серверного proxy
  Next. Корневой lint теперь проходит без предупреждений во всех контурах.
- **Индексы реестра техники**: повторно безопасная миграция добавляет составные
  индексы для фильтрации техники по компании/статусу, поиска номера внутри
  tenant, выбора последней инспекции и подсчёта дефектов. Существующий
  агрегированный API списка и его поля `lastInspection` / `defectsCount`
  сохранены; unit-тест проверяет индексы после повторной инициализации БД.
...
- **Epic 3.3.4 backend routes extraction**: region and vehicle list/detail,
  CRUD, import, archive, history and vehicle-defect routes moved from
  `backend/src/server.js` to `backend/src/routes/regions.js` and
  `backend/src/routes/vehicles.js`; shared region helpers moved to
  `backend/src/services/regions.js`.
- **Epic 3.3.4 backend routes extraction**: core inspection list/create/detail,
  update/delete and vehicle inspection history routes moved from
  `backend/src/server.js` to `backend/src/routes/inspections.js`.
...
- **Epic 3.3.4 backend routes extraction**: notifications, dashboard stats,
...
- **Epic 3.3.6 backend bootstrap extraction**: Express app wiring moved to
  `backend/src/app.js`; `backend/src/server.js` now owns only database init,
  HTTP listen, socket tracking and graceful shutdown.
- **Epic 3.3 backend app decomposition**: tenant user list/detail/create/update/
  delete routes moved from `backend/src/app.js` to `backend/src/routes/users.js`.
- **Epic 3.3 backend app decomposition**: tenant company usage and service
  notification recipient routes moved from `backend/src/app.js` to
  `backend/src/routes/companyUsage.js`.
- **Epic 3.3 backend app decomposition**: tenant settings routes moved to
  `backend/src/routes/settings.js`; photo requirement and defect category
  reference endpoints are now registered from `backend/src/routes/photo-requirements.js`.
- **Epic 3.3 backend app decomposition**: protected `/uploads/*` route moved
  from `backend/src/app.js` to `backend/src/routes/uploads.js`.
- **Epic 3.3 backend app decomposition**: company limits, feature flags and
  subscription write-guard helpers moved from `backend/src/app.js` to
  `backend/src/services/companyPolicy.js`.
- **Epic 3.3 backend app decomposition**: role predicates and authorization
...
- **Epic 3.3 backend app decomposition**: auth-sensitive rate-limit middleware
...
- **Speakeasy в web runtime — закрыто**: `speakeasy` находится только в
  `devDependencies` и используется двумя Playwright MFA-тестами. Импортов в
  `web/src` нет, production build не включает библиотеку в runtime-код.
...
- **Корневой `package-lock.json` — закрыто**: минимальный lockfile сохранён
  намеренно, потому что `launch-verify.yml` выполняет корневой `npm ci` и
  использует его как cache dependency path. Lockfile не добавляет зависимостей,
  но делает чистый CI checkout воспроизводимым.
...
- **`SECURITY.md` и `CODEOWNERS` — закрыто**: в корне добавлен стандартный
  disclosure-документ с рабочим контактом `info@auditavto.ru`, а
  `.github/CODEOWNERS` назначает подтверждённого владельца репозитория на
  backend, web, mobile, CI, security и release/data contracts. Подробная
  runtime policy остаётся в `docs/SECURITY.md`, placeholder удалён.
